...
@Component({
  selector: 'page-image-profile',
  templateUrl: 'image-profile.html'
})
export class ImageProfilePage {
          ...
          .subscribe(snapshots => {
            snapshots.forEach(snapshot => {
              this.registerForm.patchValue({
                key: snapshot.key,
                fullPath: snapshot.val().user_photo.fullPath
              })
            })
          })
      }
  ...
  update(size) {
    if (!this.registerForm.valid) {
      ...
      for (let selectedFile of [(<HTMLInputElement>document.getElementById('file')).files[0]]) {
        ...
        let fullPath = `/users/user_photo/${this.registerForm.value.key}/${selectedFile.name}`;
